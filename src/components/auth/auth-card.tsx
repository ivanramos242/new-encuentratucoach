...
function GoogleButtonPlaceholder({ mode }: { mode: "login" | "register" }) {
  return (
    <button
      ...
      aria-disabled="true"
      title="Lo configuraremos m?s adelante"
    >
      <span className="grid h-7 w-7 place-items-center rounded-full bg-white text-zinc-900">
        <i className="fa-brands fa-google" aria-hidden="true" />
      </span>
      {mode === "login" ? "Iniciar sesi?n con Google" : "Registrarme con Google"}
      <span className="text-xs font-medium text-zinc-300">(pr?ximamente)</span>
    </button>
  );
}
    ...
    <div className="fixed inset-0 z-[999999] grid place-items-center p-4">
      ...
      <div className="relative w-full max-w-xl overflow-hidden rounded-3xl border border-black/10 bg-white shadow-2xl">
        <div className="flex items-start justify-between gap-4 border-b border-black/10 bg-gradient-to-b from-cyan-50/80 to-white px-5 py-4">
          <div>
            <p className="text-lg font-black tracking-tight text-zinc-950">Restablecer contrase?a</p>
        ...
        <form
          ...
          onSubmit={(event) => {
            ...
            if (!email) {
              setStatus({ type: "error", text: "Introduce tu correo electr?nico." });
              return;
            }
            startTransition(async () => {
              ...
              try {
                ...
                setStatus({
                  type: "ok",
                  text: json.message || "Si el email existe, recibir?s instrucciones para recuperar la contrase?a.",
                });
  ...
  const rightCard = (
    <>
      <CardHeader
        ...
        hint="Inicia sesi?n con Google (recomendado) o introduce tus datos para continuar."
      />
      <div className="px-5 py-5">
          ...
          onSubmit={(event) => {
            ...
            startTransition(async () => {
              ...
              try {
                ...
                setStatus({ type: "ok", text: "Sesi?n iniciada. Redirigiendo..." });
                router.push(returnTo || (json.user?.role === "admin" ? "/admin" : "/mi-cuenta"));
                router.refresh();
              } catch (error) {
                setStatus({ type: "error", text: error instanceof Error ? error.message : "No se pudo iniciar sesi?n" });
              }
            });
          }}
        ...
        >
          ...
          <div className="grid gap-1">
            ...
            <PasswordInput name="password" placeholder="Tu contrase?a" />
          </div>
          ...
          >
            {pending ? "Entrando..." : "Iniciar sesi?n"}
          </button>
    ...
    <>
      ...
      <div className="px-5 py-5">
        ...
        >
          ...
          <div className="grid gap-1">
            <label className="text-sm font-medium text-zinc-700">Contrase?a</label>
            <PasswordInput name="password" placeholder="M?nimo 8 caracteres" />
          </div>
...
export function ForgotPasswordCard() {
  ...
  return (
    <SimpleCardShell title="Recuperar contrase?a" subtitle="Te enviaremos un enlace para restablecer tu contrase?a.">
      <form
        onSubmit={(event) => {
          event.preventDefault();
          const formData = new FormData(event.currentTarget);
...
export function ResetPasswordCard({ token }: { token: string }) {
  ...
  return (
    <SimpleCardShell title="Restablecer contrase?a" subtitle="Introduce tu nueva contrase?a para completar el acceso a tu cuenta.">
      <form
        onSubmit={(event) => {
          event.preventDefault();
          const formData = new FormData(event.currentTarget);
          ...
          if (password !== passwordConfirm) {
            setStatus({ type: "error", text: "Las contrase?as no coinciden." });
            return;
          }
          ...
          startTransition(async () => {
            ...
            try {
              ...
              setStatus({ type: "ok", text: "Contrase?a actualizada. Redirigiendo a login..." });
              setTimeout(() => {
              ...
              setStatus({
                type: "error",
                text: error instanceof Error ? error.message : "No se pudo restablecer la contrase?a",
              });
            }
          });
        }}
      ...
      >
        <label className="grid gap-1 text-sm font-medium text-zinc-800">
          Nueva contrase?a
          <PasswordInput name="password" placeholder="Nueva contrase?a" />
        </label>
        <label className="grid gap-1 text-sm font-medium text-zinc-800">
          Repite la contrase?a
          <PasswordInput name="passwordConfirm" placeholder="Repite la contrase?a" />
        </label>
        <button
          type="submit"
          disabled={pending}
          className="rounded-xl bg-zinc-950 px-4 py-2.5 text-sm font-semibold text-white hover:bg-zinc-800 disabled:opacity-60"
        >
          {pending ? "Actualizando..." : "Actualizar contrase?a"}
        </button>
        <StatusLine type={status.type} text={status.text} />
      </form>
    </SimpleCardShell>
